...
| 1.5 | Simpson's Paradox undetected — aggregate hides subgroup reversal | `detect_simpsons_paradox` in `statistics.py` — per-column stratification, masked-bias + reversal detection, small-stratum guard | 2 | ✅ |
...
| 1.8 | No reliability assessment — unstable metrics reported as fact | Three-tier flag (reliable/unstable/insufficient_data) from the bootstrap distribution; "insufficient_data" blocks verdict | 2 | ✅ |
| 1.9 | Impossibility theorem ignored — conflicting metrics presented without explanation | `detect_metric_tensions` in `statistics.py` — base-rate comparison; explains mathematically-expected tensions; `unexplained_disagreement` bool when base rates are similar (moves to `compliance/tensions.py` in Phase 5) | 2 | ✅ |
...
| 9.11 | Threshold band-splitter constant (`0.7`) was undocumented in code — the real pass boundary for the 4 gap metrics is **0.07**, not the `0.10` in `DEFAULT_THRESHOLDS`. Discovered during the `THRESHOLDS.md` audit (2026-09-02). | Documented in `THRESHOLDS.md` #6; inline `0.7` in `BiasTestSuite._get_status()` extracted to the named class constant `_PASS_BAND_FRACTION` with an explanatory comment (2026-09-02). | 2 | ✅ |
| 9.12 | `individual_fairness_score` was **mislabelled** — it measures overall model accuracy (`MetricFrame.overall`), not individual consistency ("similar people get similar predictions"). Discovered during the `THRESHOLDS.md` audit (2026-09-02). | **Renamed** `individual_fairness_score` → `overall_accuracy_floor` across `bias.py`, `statistics.py`, test files, `THRESHOLDS.md` #5, `VALIDATION_RESULTS.md`, the `detect_metric_tensions` note (2026-09-02). Pure rename — behaviour, values and validation numbers unchanged. Genuine consistency metric is 9.13. | 2 | ✅ |
...
**Discovered during implementation, tracked: 3** (9.11 ✅, 9.12 ✅ renamed,
9.13 assigned to Phase 4)
**Silently ignored: 0**
...
| 2026-09-02 | Phase 2 (partial — bootstrap confidence intervals) | 1.3 | Rhishikumar | `bootstrap_confidence_interval` + `BootstrapResult` + 5 metric wrappers + `METRIC_WRAPPERS` added to `statistics.py`. Percentile bootstrap, one shared index draw per iteration (keeps group/prediction/label aligned). Single-group resamples + metric_fn errors + non-finite values skipped-not-fatal, counted by reason in `skip_breakdown` (4th top-level field, approved: a structured always-present dict beats a warning string that only appears >5%). >5% skipped → `reliability_warning`; all-failed → nan CI + specific message, never raises. Cross-validated: n=5000 simulated gap 0.20 → point 0.208, CI [0.179, 0.234]; bootstrap std 0.014 matches analytic binomial diff-SE. pytest 34/34. **Computation done; TestResult persistence completed 2026-09-02 (Session 13 row below).** Phase 2 NOT complete — 1.1, 1.2, 1.5, 1.8, 1.9, 9.9 still open. |
| 2026-09-02 | Phase 2 (partial — multiple-comparison correction + reliability scoring) | 1.1, 1.8 | Rhishikumar | `apply_multiple_comparisons_correction` (Bonferroni scalar `alpha/n`; Benjamini-Hochberg per-rank critical list + step-up decision — `significant[i]` can be True with `p_i` > own crit, has a named test; `nan`/out-of-range/empty → `ValueError`) and `assess_reliability` (tiers reliable/unstable/insufficient_data; all rules evaluated, reasons accumulate, tier = most severe; `blocks_verdict` iff insufficient_data) added to `statistics.py`. BH validated bit-exact vs **Benjamini & Hochberg (1995) JRSS-B 57(1) Table 1** — 4 rejections vs Bonferroni's 3. **BUILD_PLAN.md Reliability Assessment section rewritten to the implemented rule table** (SD>0.05 → unstable not insufficient; min group 30 not 100; bootstrap SD not N-run reseed) with dated note + reasoning, same commit. Worked BH example in BUILD_PLAN updated to values that actually diverge. pytest 44/44. **Computation layer done; TestResult persistence completed 2026-09-02 (Session 13 row below).** Phase 2 NOT complete — 1.2, 1.5, 1.9, 9.9 still open. |
| 2026-09-02 | Phase 2 (partial — THRESHOLDS.md) | 1.2 → 🟡 | Rhishikumar | `THRESHOLDS.md` written at repo root: all 17 thresholds/constants in `bias.py` + `statistics.py` documented from source with value / lineage / owner / configurability / effect-of-change; honest limitation statement near the top; configuration roadmap describing today's reality (`__init__` override only, no persistence/audit). **1.2 stays 🟡 not ✅** — the "per-system configurable, changes audit-logged" half of its fix is a remaining Phase 2 task (matches BUILD_PLAN exit criteria which lists it separately). Two structural findings raised: **9.11** (0.7 band-splitter undocumented; real pass boundary 0.07 — 🟡, code-clarity fix tracked) and **9.12** (`individual_fairness_score` measures overall accuracy not individual consistency — ⬜, owner decision required, own session). Gap total 94 → 96. No `governance/` code changed. |
| 2026-09-02 | Phase 2 (partial — Simpson's paradox + metric tension detection) | 1.5, 1.9, 9.9 | Rhishikumar | `detect_simpsons_paradox` (per-column primitive: masked-bias + reversal detection, reversal is default-metric-only, sub-30 strata shown but non-triggering) and `detect_metric_tensions` (base-rate comparison, 3 impossibility-theorem patterns fired only when base rates differ > 0.05, `unexplained_disagreement` bool for downstream) added to `statistics.py`. `_PASS_BAND_FRACTION` now a named constant (partial 9.11). 10 new tests, 54 total green. Verified on Adult Income (no false paradox) + COMPAS (base rates differ 0.133 as literature expects; all-fail → no tension pattern, correct). BUILD_PLAN 2.3/2.4 + THRESHOLDS.md #18 updated in commit `a57abee`. Phase 2 remaining: 1.2 config-half, the `TestResult` wiring sub-step, 9.12 decision. |

| 2026-09-02 | Phase 2 (rename session) | 9.11, 9.12 | Rhishikumar | `individual_fairness_score` → `overall_accuracy_floor` across `bias.py` / `statistics.py` / both test files / `THRESHOLDS.md` #5 / `VALIDATION_RESULTS.md` / `detect_metric_tensions` note. **Pure rename** — `notebooks/validate_phase1.py` reproduces 0.8440 / 0.6263 / 0.7467 bit-identically; all 54 tests pass. Same commit: inline `0.7` in `_get_status()` → `_PASS_BAND_FRACTION` class constant + comment (9.11 code-clarity fix; `value * 0.7` and `value * _PASS_BAND_FRACTION` evaluate identically — confirmed by the bit-identical validation and the band-boundary test `test_custom_threshold_override_changes_outcome`). One-line comment added noting `by_group` accuracy in the metric detail is informational only, not part of the verdict. |
| 2026-09-02 | Phase 2 Part A — TestResult statistical wiring (Session 13) | — (1.3/1.4/1.8 persistence; no new gap rows) | Rhishikumar | First session to touch `engine.py` / DB schema this phase. **`permutation_p_value`** (new fn, additive — `significance_test` untouched; bit-for-bit matches it on demographic parity across seeds). **`TestResult` +7 nullable columns** + `scripts/migrate_add_testresult_stats.py` (idempotent `ALTER TABLE`, tested on synthetic old-schema DB + run against the real, empty `ai_governance.db`). **`engine.py` wiring** — bootstrap CI + reliability tier + sample_size for all 5 metrics; permutation p-value + Bonferroni-corrected threshold for the **4 fairness-gap metrics as one family per attribute**; `overall_accuracy_floor` excluded from correction (group-agnostic → permutation p-value structurally meaningless; those 3 columns NULL) — **owner-confirmed 4-vs-5 split**, BUILD_PLAN.md corrected. `status="indeterminate"` (engine-set) when reliability blocks; `is_pass()` guard rail in `models.py`. **Key finding: runtime is iteration-bound, not size-bound** (~9×1000-iter loops/attr ≈ 9000 fairlearn calls at fixed overhead; n=20 and n=200 cost the same ~38s) — matters for Phase 3+ which add more statistical tests. Fix: autouse fixture in `test_engine.py`/`test_api_testing.py` only, iters 1000→100 for plumbing tests; value-asserting tests in `test_engine_statistics.py` (`@pytest.mark.statistical`, real 1000). `bias.py` "5 in fixed order" contract unchanged. Commits `c6d3640`, `188fbe0`, + Step 3. |

**Phase 2 status after this session: only Part B (per-system threshold config +
audit log) remains before the Phase 3 gate opens.** 1.2 stays 🟡 — THRESHOLDS.md
is done and the statistics are now persisted, but the "configurable per system,
changes audit-logged" half of 1.2's fix is Part B, next session.
